...
def create_interview_service(data: CreateInterviewRequest, db: Session) -> CreateInterviewResponse:
...
# 多轮会话.
def submit_answer_service(interview_id: str, data: SubmitAnswerRequest, db: Session) -> SubmitAnswerResponse:
    now = datetime.now()
    ...
    # 助手最后一条回复.
    last_question_message = db.execute(
    ...
    user_message = InterviewMessage(interview_id=interview_id, role="user", content=data.answer, round=current_round, created_at=now)
    db.add(user_message)
    ...
    response = invoke(build_follow_up_question_prompt(last_question, data.answer, current_round))

    next_round = current_round + 1
    assistant_message = InterviewMessage(interview_id=interview_id, role="assistant", content=response, round=next_round, created_at=now)
...
# 面试反馈与总结.
def finish_interview_service(interview_id: str, data: FinishInterviewRequest, db: Session) -> FinishInterviewResponse:
    interview = db.get(Interview, interview_id)
